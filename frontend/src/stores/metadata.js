...
export const useMetadataStore = defineStore('metadata', {
	...
	actions: {
      ...
      async publish( ) {
         ...
         return axios.post( `${system.jobsURL}/metadata/${this.detail.id}/publish` ).then( () => {
            if ( this.detail.dateDLIngest ) {
               this.detail.dateDLUpdate = new Date()
            } else {
               this.detail.dateDLIngest = new Date()
            }
         }).catch( e => {
            system.setError(e)
         })
      ...
      async publishToArchivesSpace( userID ) {
         const system = useSystemStore()
         const payload = {user: userID, review: false}
         return axios.post(`/api/metadata/${this.detail.id}/archivesspace/publish`, payload).then( () => {
